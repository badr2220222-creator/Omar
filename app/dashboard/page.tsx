...
const DashbaordPage = async () => {
  ...
  const t = await getTranslations('dashboardlinks');
    ...
    <>
      <div className="flex items-center justify-between px-2">
        <div className="sm:grid gap-1 hidden">
          <h1 className="font-heading text-3xl md:text-4xl">{t("eventtypes")}</h1>
          <p className="text-lg text-muted-foreground">
            {te("createandmanage")}
          </p>
        </div>
        <Button asChild>
          <Link href="/dashboard/new">{te("createnewevent")}</Link>
        </Button>
      </div>
          ...
          {data.EventType.map((item) => (
            ...
            >
              <div className="absolute top-2 right-2">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="w-20" align="end">
                    <DropdownMenuLabel>{te("event")}</DropdownMenuLabel>
                    <DropdownMenuSeparator />
                    <DropdownMenuGroup>
                      <DropdownMenuItem asChild>
                        <Link href={`/${data.username}/${item.url}`}>
                          <ExternalLink className="mr-2 h-4 w-4" />
                          <span>{te("preview")}</span>
                        </Link>
                      </DropdownMenuItem>
                      <CopyLinkMenuItem
                    ...
                    <div className="ml-5 w-0 flex-1">
                      <dl>
                        <dt className="text-sm font-medium truncate ">
                          {item.duration} {te("minutesmeeting")}
                        </dt>
                        <dd>
              ...
              <div className="bg-muted dark:bg-gray-900 px-5 py-3 flex justify-between items-center">
                ...
                <Link href={`/dashboard/event/${item.id}`}>
                  <Button className="">{te("editevent")}</Button>
                </Link>
              </div>
            </div>
          ))}
